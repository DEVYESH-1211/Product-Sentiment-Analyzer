...
interface SummaryPanelProps {
  reviews: Review[];
  // Backend /summary payload. Optional + falls back to computing from
  // `reviews` so this component still works even before /summary field
  // names are fully confirmed — see src/api/types.ts.
...
export const SummaryPanel: React.FC<SummaryPanelProps> = ({ reviews, summary }) => {
  ...
  const positive =
    typeof summary?.positive_count === 'number'
      ? summary.positive_count
      : reviews.filter(r => r.sentiment === 'positive').length;

  const neutral =
    typeof summary?.neutral_count === 'number'
      ? summary.neutral_count
      : reviews.filter(r => r.sentiment === 'neutral').length;

  const negative =
    typeof summary?.negative_count === 'number'
